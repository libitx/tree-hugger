...
class MetaNode {
  ...
  get opReturn() {
    const output = this.outputs.find(o => {
      if (o.tape.length) {
        const firstCell = o.tape[0].cell;
        const lastOP = firstCell[firstCell.length - 1];
        return lastOP ? lastOP.op === 106 : false;
      }
    });

    return output ? output.tape : null;
  }
